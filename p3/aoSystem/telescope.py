...
class telescope:
    ...
    @property
    def R(self):
        """Telescope radius in meters [m]"""
        return self.D/2
    @property
    def area(self):
        """Telescope area in meter square [m^2]"""
    ...
    @property
    def airmass(self):
        return 1.0 / np.cos(self.zenith_angle*np.pi / 180.0)
    ...
    def __init__(self,D,resolution, zenith_angle=0.0,obsRatio=0.0,pupilAngle = 0.0,\
        ...
        def check_path(path):
            return (path is not None) and (ospath.isfile(path)) and (re.search(".fits", path) is not None)
